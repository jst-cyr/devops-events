...
const EXCLUDED_SHORTLIST_EMBEDDED_PATTERN =
  /\bembedded\b|\bfirmware\b|\brtos\b|\byocto\b|embedded\s+linux|embedded-linux|\belc\b|automotive\s+grade\s+linux|\bagl\b/i;
...
function getShortlistExclusionReason(record) {
  ...
  if (EXCLUDED_SHORTLIST_EMBEDDED_PATTERN.test(searchText)) {
    return "excluded topic: embedded/firmware scenario";
  }
